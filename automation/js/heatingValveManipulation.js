rules.JSRule({
  ...
  triggers: [triggers.GroupStateUpdateTrigger('gMan', null, null)],
  execute: (event) => {
    ...
    function toggleValve(item, turnON) {
      LogAction.logInfo("plug", "Preverjam ali je potrebno odpreti ventil");
      if (turnON) {
        LogAction.logInfo("heating", "Potrebno je ogreti sobo");
        if (item.state == "OFF" && item.groupNames.filter(group => group === "gFhsRev").length == 0) {
          item.sendCommand("ON");
          LogAction.logInfo("heating", "Odpiram nc venitl {}", item.name)
        } else if (item.state == "ON" && !item.groupNames.filter(group => group === "gFhsRev").length == 0) {
          item.sendCommand("OFF");
          LogAction.logInfo("heating", "Odpiram no venitl {}", item.name);
        }
      } else {
        LogAction.logInfo("heating", "Ni potrebno ogreti sobe");
        if (item.state == "ON" && item.groupNames.filter(group => group === "gFhsRev").length == 0) {
          item.sendCommand("OFF");
          LogAction.logInfo("heating", "Zapiram nc venitl {}", item.name)
        } else if (item.state == "OFF" && !item.groupNames.filter(group => group === "gFhsRev").length == 0) {
          item.sendCommand("ON");
          LogAction.logInfo("heating", "Zapiram no venitl {}", item.name);
        }
      }
    }
    ...
    var manValve = items.getItem(event.itemName);
    if (manValve != null) {
      valveName = event.itemName.substring(3);
      LogAction.logInfo("heating", "Ročni ventil {} je bil spremenjen", valveName);
      var fhsItem = items.getItem("gFhs").members.filter(loopFhsItem => loopFhsItem.name.includes(valveName))[0];
      ...
      if (difItem != "undefined" && difItem != null) {
        ...
        if (manValve.state == "ON" && difTemp < 0.0) {
          ...
          toggleValve(fhsItem, false);
          if (Heating.state == "ON" && gMan.state == "OFF") {
            Heating.sendCommand("OFF");
          }
          // if (!noVent) fhsItem.sendCommand(OFF) else if (Man.state == ON)fhsItem.sendCommand(ON)
        }

      } else LogAction.logInfo("heating", "Nisem našel diffa za {}", valveName);
