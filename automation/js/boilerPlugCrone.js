rules.JSRule({
  ...
  execute: (event) => {
      ...
      if (items.getItem("Away").state != "ON") {
        ...
        if (plugBoilerSwitch.state != "ON") {  
          ...
          boilerCheckPowerTimer = actions.ScriptExecution.createTimer(x.plusSeconds(60),() => {
            ...
            LogAction.logDebug("boiler", "Preverjam vrednost odjema elektrike {}", items.getItem("plugBoilerPower").rawState.intValue());
            if (items.getItem("plugBoilerPower").rawState.intValue() < 100) {
              ...
              LogAction.logDebug("boiler", "Vrednost je večja od 100 zato končujem timer.");
            }
          ...
          if (Ephemeris.isWeekend() || Ephemeris.isBankHoliday()) {
            LogAction.logDebug("boiler", "Bojler je že prižgan ampak je ali praznik ali vikend in je to OK!");
          } else {
    ...
    function boilerOff() {
      ...
      if (Ephemeris.isWeekend() || Ephemeris.isBankHoliday()) {
        LogAction.logDebug("boiler", "Ker je vikend ali praznik, ne ugašam bojlerja, da jih ne bom spet slišal");
      } else {
        if (plugBoilerSwitch.state != "OFF") {
          ...
          LogAction.logDebug("boiler", "Pred preverjanjem trenutne moči bojlerja - {} je trenutno stanje ure za ugašanje {}:{} ", items.getItem("plugBoilerPower").rawState, hours, minutes);
          if (items.getItem("plugBoilerPower").rawState > 0) {
            LogAction.logDebug("boiler", "Ker bojler še vedno ogreva vodo, premaknem uro prižiga 10 min nazaj.");
            if (minutes == 0) {
            ...
            LogAction.logDebug("boiler", "Ker bojler ne ogreva več vode, premaknem uro prižiga 5 min naprej.");
            if (minutes == 55) {
          ...
          LogAction.logDebug("boiler", "Nastavil sem novo uro prižiga - " + hoursStartItem.state + ":" + minutesStartItem.state);
