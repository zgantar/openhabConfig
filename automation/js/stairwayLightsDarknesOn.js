rules.JSRule({
  ...
  execute: (event) => {
    ...
    if (lum.rawState.intValue() <= 8) {
      LogAction.logInfo("lights", "Ker je sprememba zunanje osvetljenosti na - {} manjša od 8, preverjam stanje luči.", lum.state);
      if (lightSwitch.state == "OFF") {
        if (lum.history.previousState > lum.state) {
          ...
          lightsCheckPowerTimer = actions.ScriptExecution.createTimer(x.plusSeconds(60),() => {
            ...
            LogAction.logInfo("lights", "Preverjam vrednost odjema elektrike {}", items.getItem("plugLiSt2Switch2Power").rawState);
            if (items.getItem("plugLiSt2Switch2Power").rawSate < 5) {
