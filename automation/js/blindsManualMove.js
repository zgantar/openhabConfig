rules.JSRule({
  ...
  execute: (event) => {
    ...
    LogAction.logInfo("blinds", "In to je item {}.", triggeringItem.type);
    ...
    if (stateItem != null) {
      ...
      if (triggeringItem.state == "OFF") {
        var prevoiusState = event.oldState;
        LogAction.logInfo("blinds", "oldState je {} ", prevoiusState);
        //var lastUpdateTime = PersistenceExtensions.lastUpdate(triggeringItem, "jdbc").toEpochSecond(); //triggeringItem.history.lastUpdate().toEpochSecond();// //.getTimeStamp().toEpochSecond(); //.history.previousState .timestamp.toInstant().toEpochMilli()
        //LogAction.logInfo("blinds", "Zadnja sprememba je bila {} ", lastUpdateTime);
        var lastUpdateTime = x.toEpochSecond();
